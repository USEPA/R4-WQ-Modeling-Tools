...
Public Class D4EMMetCmp
    Public DataSets As atcTimeseriesGroup
    ...
    ''' <summary>
    ''' This routine contains the disaggregate of data from daily to hourly according to -OBS timeseries if available
    ''' also contains ET calculation for Jensen, Hamon, and Penman Pan evap
    ''' </summary>
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Open(ByVal aOperationName As String, _
        ...
        Dim lRainTSer As atcTimeseries = Nothing
        ...
        Select Case aOperationName
            Case "Solar Radiation"
                ...
                If lOk AndAlso lCldTSer IsNot Nothing AndAlso _
                  lLatitude >= MetComputeLatitudeMin AndAlso lLatitude <= MetComputeLatitudeMax Then
                    Dim lMetCmpTS As atcTimeseries = SolarRadiationFromCloudCover(lCldTSer, Nothing, lLatitude)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Cloud Cover from Solar"
                ...
                If lOk AndAlso lSRadTSer IsNot Nothing AndAlso _
                  lLatitude >= MetComputeLatitudeMin AndAlso lLatitude <= MetComputeLatitudeMax Then
                    Dim lMetCmpTS As atcTimeseries = CloudCoverTimeseriesFromSolar(lSRadTSer, Nothing, lLatitude)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Jensen PET"
                Dim lCTX As Double
                ...
                If lOk AndAlso lTMinTSer IsNot Nothing AndAlso lTMaxTSer IsNot Nothing AndAlso _
                    ...
                    Dim lMetCmpTS As atcTimeseries = CmpJen(lTMinTSer, lTMaxTSer, lSRadTSer, Nothing, lDegF, lCTX, lCTS)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Hamon PET"
                If aArgs Is Nothing Then
                ...
                If lOk AndAlso lTMaxTSer IsNot Nothing AndAlso lTMinTSer IsNot Nothing AndAlso _
                   lLatitude >= MetComputeLatitudeMin AndAlso lLatitude <= MetComputeLatitudeMax Then
                    Dim lMetCmpTS As atcTimeseries = PanEvaporationTimeseriesComputedByHamon(lTMinTSer, lTMaxTSer, Nothing, lDegF, lLatitude, lCTS)
                    DataSets.Add(lMetCmpTS)
                End If
            ...
            Case "Wind Travel"
                ...
                If lWindTSer IsNot Nothing Then
                    Dim lMetCmpTS As atcTimeseries = WindTravelFromWindSpeed(lWindTSer, Nothing)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Cloud Cover"
                ...
                If Not lPctSunTSer Is Nothing Then
                    Dim lMetCmpTS As atcTimeseries = CmpCld(lPctSunTSer, Nothing)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Solar Radiation (Disaggregate)"
                ...
                If lOk AndAlso lDlyTSer IsNot Nothing AndAlso _
                  lLatitude >= MetComputeLatitudeMin AndAlso lLatitude <= MetComputeLatitudeMax Then
                    Dim lMetCmpTS As atcTimeseries = DisSolPet(lDlyTSer, Nothing, 1, lLatitude)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Evapotranspiration"
                ...
                If lOk AndAlso lDlyTSer IsNot Nothing AndAlso _
                  lLatitude >= MetComputeLatitudeMin AndAlso lLatitude <= MetComputeLatitudeMax Then
                    Dim lMetCmpTS As atcTimeseries = DisSolPet(lDlyTSer, Nothing, 2, lLatitude)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Temperature"
                If aArgs Is Nothing Then
                ...
                If lOk AndAlso lTMinTSer IsNot Nothing AndAlso lTMinTSer IsNot Nothing AndAlso lObsTimeTSer IsNot Nothing Then
                    Dim lMetCmpTS As atcTimeseries = DisaggTemp(lTMinTSer, lTMaxTSer, Nothing, lObsTimeTSer)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Wind (Disaggregate)"
                ...
                If lOk AndAlso lWindTSer IsNot Nothing AndAlso Math.Abs(lHrSum - 1) < 0.001 Then
                    Dim lMetCmpTS As atcTimeseries = DisWnd(lWindTSer, Nothing, lHrDist)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Precipitation"
                ...
                If lOk AndAlso lHrTSers IsNot Nothing AndAlso lObsTimeTSer IsNot Nothing And _
                    ...
                    Dim lMetCmpTS As atcTimeseries = DisaggPrecip(lDlyTSer, Nothing, lHrTSers, lObsTimeTSer, lTol, lSummFile)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "Dewpoint"
                Dim lDewPTSer As atcTimeseries = Nothing
                ...
                If lOk AndAlso lDewPTSer IsNot Nothing AndAlso lATmpTSer IsNot Nothing Then
                    Dim lMetCmpTS As atcTimeseries = DisDewPoint(lDewPTSer, Nothing, lATmpTSer)
                    DataSets.Add(lMetCmpTS)
                End If
            Case "SWATPMET"
                Dim lElevation As Double = aArgs.GetValue("Elevation") 'in meter
                lLatitude = aArgs.GetValue("Latitude", 39)
                Dim lLongitude As Double = aArgs.GetValue("Longitude", -999)
                If lLongitude > 0 Then lLongitude *= -1
                lRainTSer = aArgs.GetValue("DailyRain") 'in mm/daily
                lTMinTSer = aArgs.GetValue("TMIN") 'in degree C/daily
                lTMaxTSer = aArgs.GetValue("TMAX") 'in degree C/daily
                Dim lRetainDailyTimeStep As Boolean = aArgs.GetValue("RetainDailyTimeStep", False)
                lOk = True
                Dim PETstationUseClosest As String = "(Closest)"
                Dim PETstationID As String = PETstationUseClosest
                Dim PETstation As SwatWeatherStation = Nothing
                Dim PETswatStations As New SwatWeatherStations
                Dim lStationID As String = PETstationID.ToLower
                For Each lSearchStation As SwatWeatherStation In PETswatStations
                ...
                If PETstation Is Nothing Then
                    Throw New ApplicationException("D4EM: PET station not found")
                ElseIf Double.IsNaN(lElevation) Then
                    Throw New ApplicationException("D4EM: Elevation not found for PET")
                Else
                    If lOk AndAlso lRainTSer IsNot Nothing AndAlso lTMinTSer IsNot Nothing AndAlso lTMaxTSer IsNot Nothing Then
                        'lElevation = PETstation.Elev
                        Dim lMetCmpTS As atcTimeseries = PanEvaporationTimeseriesComputedByPenmanMonteith( _
                                    lElevation, _
                                    lRainTSer, _
                                    lTMinTSer, _
                                    lTMaxTSer, _
                                    Nothing, PETstation, , 330.0)
                        If lMetCmpTS.Attributes.GetValue("tu") = 4 AndAlso Not lRetainDailyTimeStep Then
                            'Disaggragate the daily SWAT PMET timeseries into hourly
