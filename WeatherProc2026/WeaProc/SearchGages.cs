...
namespace NCEIData
{
    class SearchGages
    {
            ...
            if (lstGages.Count > 0) lstGages.Clear();
            {
                ...
                switch (lDatasource)
                {
                    case (int)DataSource.ISD:
                        ...
                        qrys.Append("&outFields=STATION_ID%2CGHCNH_ID%2CWBAN_ID%2CSTATION_NAME%2CGHCNH_POR_START%2CGHCNH_POR_END%2CLATITUDE%2CLONGITUDE%2CELEVATION%2CSTATE");
                ...
                switch (lDatasource)
                {
                    case (int)DataSource.ISD:
                        lstAWS = mainObj["features"].Select(m =>
                            (string)m.SelectToken("attributes.GHCNH_ID")).ToList();
                            //(string)m.SelectToken("attributes.AWS")).ToList();
                        lstWBAN = mainObj["features"].Select(m =>
                            (string)m.SelectToken("attributes.WBAN_ID")).ToList();
                            //(string)m.SelectToken("attributes.WBAN")).ToList();
                        lstBegDate = mainObj["features"].Select(m =>
                            (string)m.SelectToken("attributes.GHCNH_POR_START")).ToList();
                        lstEndDate = mainObj["features"].Select(m =>
                            (string)m.SelectToken("attributes.GHCNH_POR_END")).ToList();
                    ...
                    try
                    {
                        ...
                        switch (lDatasource)
                        {
                            case (int)DataSource.ISD:
                                ...
                                if (!(lstWBAN.ElementAt(i) == null))
                                    sta = lstWBAN.ElementAt(i).ToString();
        ...
        {
            // Updated to process as date as offset rather than specific date
            // ISD date is in unix milliseconds since 1970-01-01
            long unixMillis = long.Parse(isdDate);
            // .NET Framework does not have DateTimeOffset.FromUnixTimeMilliseconds
            // Instead, add milliseconds to Unix epoch manually
            DateTime epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
            DateTime dto = epoch.AddMilliseconds(unixMillis);
            return dto;
        }
