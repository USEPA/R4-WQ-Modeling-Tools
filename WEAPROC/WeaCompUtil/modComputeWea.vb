...
Public Module modComputeWea
    ...
    Public Function DisaggDates(ByVal aInTS As atcTimeseries, ByVal aDataSource As atcTimeseriesSource) As atcTimeseries
    ...
    ''' <remarks></remarks>
    Public Function DewpointTimeseriesFromSpecificHumidity(ByVal aSpecHumidTSer As atcTimeseries,
    ...
    Public Function DewpointTimeseriesFromRelativeHumidity(ByVal aRelHumidTSer As atcTimeseries,
                                                                ByVal aAirTempTSer As atcTimeseries,
                                                                ByVal aSource As atcTimeseriesSource,
                                                                Optional ByVal aPressure As Double = 1013.25) As atcTimeseries

        Dim lTSGroup As New atcDataGroup(aRelHumidTSer, aAirTempTSer)
        Dim lDewpointTS As New atcTimeseries(aSource)
        CopyBaseAttributes(aRelHumidTSer, lDewpointTS)
        lDewpointTS.Attributes.SetValue("Constituent", "DDEW")
        lDewpointTS.Attributes.SetValue("TSTYPE", "DDEW")
        lDewpointTS.Attributes.SetValue("Scenario", "COMPUTED")
        lDewpointTS.Attributes.SetValue("Description", "Dewpoint temperature (Deg F) computed from Relative Humidity (%)")
        lDewpointTS.Attributes.Add("RELHUM", aRelHumidTSer.ToString)
        'lDewpointTS.Dates = atcData.MergeDates(lTSGroup)
        lDewpointTS.Dates = aAirTempTSer.Dates
        lDewpointTS.numValues = lDewpointTS.Dates.numValues
        ...
        Dim RelHumid As Double
        Dim lDate(5) As Integer
        Dim lRelHumidIndex As Integer
        Dim lAirTempIndex As Integer
        Dim lPoint As Boolean = aRelHumidTSer.Attributes.GetValue("point", False)
        For lValueIndex As Integer = 1 To lDewpointTS.numValues
            lRelHumidIndex = FindDateAtOrAfter(aRelHumidTSer.Dates.Values, lDewpointTS.Dates.Value(lValueIndex), lRelHumidIndex)
            lAirTempIndex = FindDateAtOrAfter(aAirTempTSer.Dates.Values, lDewpointTS.Dates.Value(lValueIndex), lAirTempIndex)
            If lPoint Then
                Call J2Date(lDewpointTS.Dates.Value(lValueIndex), lDate)
            Else
            ...
            'e = aSpecHumidTSer.Value(lSpecHumidIndex) * aPressure / (0.378 * aSpecHumidTSer.Value(lSpecHumidIndex) + 0.622)
            'RelHumid = e / es
            RelHumid = aRelHumidTSer.Value(lValueIndex) / 100.0
            If RelHumid > 1.0 Then
                RelHumid = 1.0
            ElseIf RelHumid < 0 Then
                RelHumid = 0.0
            End If
            'lDewpointTS.Value(lValueIndex) = aAirTempTSer.Value(lAirTempIndex) - ((100 - 100 * RelHumid) / 5)
            lDewpointTS.Value(lValueIndex) = aAirTempTSer.Value(lValueIndex) - ((100 - 100 * RelHumid) / 5)
        Next lValueIndex
...
#Region "From atcMetCmp"
    ...
    Private Function CloudCoverValueFromSolar(ByRef aDegLat As Double, ByRef aDayRad As Double, ByRef aMon As Integer, ByRef aDay As Integer) As Double
        ...
        If ss < 0.0# Then 'can't have SS being negative
            'Debug.WriteLine("ss < 0: {0},{1},{2}", aMon.ToString(), aDay.ToString(), ss.ToString())
            ss = 0.0#
            'fix 1/18/2021
        ElseIf (ss > 100.0#) Then
            'Debug.WriteLine("ss > 100 0: {0},{1},{2}", aMon.ToString(), aDay.ToString(), ss.ToString())
            ss = 100.0#
        ElseIf (Double.IsNaN(ss)) Then
            'Debug.WriteLine("ss IsNan: {0},{1},{2}", aMon.ToString(), aDay.ToString(), ss.ToString())
            ss = 0.0#
        End If

        'debug cloud
        'Debug.WriteLine("SOLR,Y100,YRDb,II,YRD,SS,CLOU")
        'If aMon > 6 And aMon < 10 Then
        'Dim dclou As Double = 10 * Math.Pow(-((ss / 100) - 1), 3 / 5)
        'Debug.WriteLine("{0},{1},{2},{3},{4},{5},{6}",
        'aDayRad.ToString(), Y100.ToString(),
        'YRDbef.ToString(), ii.ToString(),
        'YRD.ToString(), ss.ToString(), dclou.ToString())
        'End If
